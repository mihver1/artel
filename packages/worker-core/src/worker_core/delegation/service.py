...
from worker_core.bootstrap import bootstrap_runtime, create_agent_session_from_bootstrap
from worker_core.config import WorkerConfig
from worker_core.delegation.models import DelegatedRun
from worker_core.delegation.registry import get_registry
from worker_core.extensions import ExtensionContext
from worker_core.tools.builtins import create_all_tools, create_readonly_tools

_DELEGATE_PROMPT = (
    "You are a delegated Artel worker operating inside the current session context. "
...
class DelegationService:
    ...
    def config(self) -> WorkerConfig:
        if isinstance(getattr(self.context, "config", None), WorkerConfig):
    ...
    def _config_for_mode(self, mode: str) -> WorkerConfig:
