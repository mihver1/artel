...
This document operationalizes `Artel PRD v0` into an execution-ordered backlog. The goal is to turn Worker into Artel with a full product rename, first-run migration, cmux-only bootstrap, built-in official capabilities, and an orchestration-first workspace with dashboard, orchestrator, and employee surfaces.
## Current implementation anchors
Artel-first bootstrap, config resolution, and compatibility handling are now in place. The repository ships with an `artel` primary entrypoint, the `src/artel` meta-package, Artel-first generated config/templates, and Artel-first docs/install surfaces, while temporary compatibility bridges remain through the `worker` CLI alias, legacy env/path fallbacks, and internal `worker_*` implementation imports.
Persisted state is now Artel-first across global/project config, auth, prompts, skills, themes, local server state, extension manifests, provider overlay, and web/admin/runtime copy, with legacy Worker fallback reads intentionally preserved where migration still depends on them. The broad user-facing rename sweep across README/docs, installer/help text, runtime strings, and rename-sensitive tests is complete and validated.
...
Workstream 2 is partially complete: root metadata, the primary `artel` entrypoint, the `src/artel` package, the alias-package compatibility bridge, and the broad user-facing rename sweep are done. Artel-first entry-point group discovery with Worker fallback is also in place; the physical `worker_*` package/module rename and first-party import flip remain deferred.
Workstream 3 is largely complete: config templates, env resolution, `.artel` project generation, prompts/skills/themes lookup, installer behavior, managed local server tokens, and Artel-first generated artifacts are in place with legacy Worker fallbacks preserved where needed.
...
Workstream 5 is partially complete: the shared workspace-summary module and tests are in place and the web layer already delegates much of its follow/task/file/diff/terminal/tool summarization to core, but `packages/worker-web/src/worker_web/rendering.py` still contains duplicate formatting and summary-adjacent heuristics that should be collapsed further into core-backed helpers so the web layer becomes a thinner presenter.
...
1. Remove the remaining duplicate workspace-summary heuristics from `packages/worker-web/src/worker_web/rendering.py` so the web layer becomes a thinner consumer of shared core helpers.
2. Expand `worker_core.workspace_summary` with any remaining pure formatting/selection helpers needed by web surfaces and lock them down with tests before building new UI on top.
...
2. Introduce explicit Artel path/constants definitions for global config dir, auth file, session DB, extension manifest, registry cache, provider overlay, prompts, skills, and migration state while preserving temporary read access to the legacy Worker paths.
3. Extend the migration system in `packages/worker-core/src/worker_core/migrations.py` so it can manage real versioned data migrations instead of only config-version tracking.
4. Add idempotent migration helpers for global state rooted in `~/.config/worker`: `config.toml`, `auth.json`, `sessions.db`, `extensions.lock`, `registry_cache`, `server-provider-overlay.json`, `prompts`, `skills`, `SYSTEM.md`, `APPEND_SYSTEM.md`, and `state.json`.
5. Add idempotent migration helpers for project-local state rooted in `.worker`: `config.toml`, `AGENTS.md`, `SYSTEM.md`, `APPEND_SYSTEM.md`, `prompts`, `skills`, `server.json`, and `mcp.json` where present.
...
7. Add temporary compatibility reads so Artel can still consume legacy Worker state when migration has not yet occurred or only partially completed.
...
1. Rename the root project metadata in `pyproject.toml` from Worker to Artel, including the project name, workspace members, uv source aliases, and top-level package metadata.
2. Rename the root meta-package from `src/worker` to `src/artel`.
3. Rename workspace package directories and project metadata from `worker-ai`, `worker-core`, `worker-server`, `worker-tui`, and `worker-web` to Artel equivalents.
4. Introduce temporary `artel_*` alias packages so Artel import paths can exist before the physical module-tree rename.
5. Keep internal first-party implementation imports on `worker_*` until the real module rename lands; switching internal imports early creates duplicate Python module identities under both names and breaks registries, monkeypatches, and persisted/runtime lookups.
6. Rename the physical Python module packages from `worker_ai`, `worker_core`, `worker_server`, `worker_tui`, and `worker_web` to Artel equivalents.
...
8. Replace the main CLI entrypoint with `artel` and add only the minimum temporary compatibility alias required to bridge the transition from existing Worker installs.
9. Rename all first-party entry-point group names from `worker.extensions`, `worker.tui`, `worker.server`, `worker.ai`, and `worker.web` to Artel names while preserving temporary compatibility discovery for legacy third-party plugins.
...
11. Update all tests, fixtures, and import-time monkeypatches that refer to Worker package names or Worker-specific paths.
12. Run a repository-wide sweep for residual Worker-specific package/import references and resolve intentional compatibility leftovers explicitly.
## Workstream 3 — config, environment, installer, and generated artifact rename
1. Rename generated config templates and comments in `packages/worker-core/src/worker_core/config.py (213-577)` from Worker to Artel.
2. Rename config/env variables such as `WORKER_CONFIG_DIR`, `WORKER_INSTALL_DIR`, `WORKER_BIN_DIR`, and Worktree-related Worker env names to Artel equivalents while preserving temporary fallback reads of the old names.
3. Rename project-local directory generation from `.worker` to `.artel` and update generated `AGENTS.md`/context files accordingly.
4. Rename prompt and skill discovery roots from Worker paths to Artel paths while preserving temporary legacy fallback reads.
5. Rename OAuth auth storage defaults from Worker paths to Artel paths and update error/help text that still says `worker login`.
6. Rename provider overlay, managed local server registry, and any other generated runtime artifacts from Worker-specific paths to Artel-specific paths.
...
8. Add installer and config-generation tests that cover new Artel paths plus legacy Worker migration behavior.
...
3. Move the current heuristics from `packages/worker-web/src/worker_web/rendering.py` into reusable pure functions that do not depend on NiceGUI.
4. Adapt the web layer to consume the shared summary module instead of its private Worker-specific rendering logic.
...
2. Move the current worktree service behavior from `/Users/m.verhovyh/Projects/worker-ext-worktree/src/worker_ext_worktree/service.py` into first-party Artel code and update naming/path conventions from Worker to Artel.
3. Move the current MCP config/runtime behavior from `/Users/m.verhovyh/Projects/worker-ext-mcp/src/worker_ext_mcp` into first-party Artel code, including Artel path support for `.artel/mcp.json` and global Artel config roots.
4. Move the current subagent registry, runner, server endpoints, and TUI integration from `/Users/m.verhovyh/Projects/worker-ext-subagents/src/worker_ext_subagents` into first-party Artel code.
...
2. Remove or de-emphasize the old Worker generic chat framing so the orchestrator surface behaves as the command console for delegation and coordination.
...
3. Update any remaining web UI branding and shared-control labels from Worker to Artel.
...
5. Add migration notes for existing Worker users and plugin authors.
## Validation gates
...
Before completion, the renamed repository must pass unit tests and integration tests for migration, bootstrap, orchestration, and dashboard behavior, and the user-facing docs/install flow must describe Artel rather than Worker.