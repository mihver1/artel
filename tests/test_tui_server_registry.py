...
def test_server_registry_round_trip(monkeypatch, tmp_path):
    ...
    upsert_saved_server(SavedArtelServer(name="Prod", remote_url="ws://prod:7432", auth_token="tok"))
    upsert_saved_server(SavedArtelServer(name="Prod EU", remote_url="ws://prod-eu:7432", auth_token="tok2"))
...
def test_server_registry_upsert_replaces_same_url(monkeypatch, tmp_path):
    from worker_core import config as cfg_mod
    from worker_tui.server_registry import SavedArtelServer, load_saved_servers, upsert_saved_server

    fake_config = tmp_path / "config"
    monkeypatch.setattr(cfg_mod, "CONFIG_DIR", fake_config)

    upsert_saved_server(SavedArtelServer(name="Prod", remote_url="ws://prod:7432", auth_token="old"))
    upsert_saved_server(SavedArtelServer(name="Production", remote_url="ws://prod:7432", auth_token="new"))
